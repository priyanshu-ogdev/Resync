...
| Shipped and installable | `pip install resync-mcp` succeeds from a clean environment; flagship demo runs end to end | Built and packaged — standalone C installer (`installer/resync.exe`), POSIX installer (`installer/resync.sh`), and `pyproject.toml` distribution with `doctor` verification |
