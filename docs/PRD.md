...
| Shipped and installable | `pip install resync-mcp` succeeds from a clean environment; flagship demo runs end to end | Built and packaged — standalone C installer (`installer/install.exe`), POSIX installer (`installer/install.sh`), and `pyproject.toml` distribution with `doctor` verification |
