...
from resync.resolve.resolver import ResolvedDependency, ResolverError, ResolveResult, ResolverUnavailableError
...
def test_resolve_exits_2_when_resolver_unavailable(tmp_path: Path) -> None:
    with patch("resync.resolve.resolver.resolve", side_effect=ResolverUnavailableError("network down")):
        runner = CliRunner()
...
def test_resolve_exits_1_on_genuine_resolver_error(tmp_path: Path) -> None:
    with patch("resync.resolve.resolver.resolve", side_effect=ResolverError("no solution found")):
        runner = CliRunner()
